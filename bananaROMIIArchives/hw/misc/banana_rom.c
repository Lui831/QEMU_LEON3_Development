...
#include "hw/irq.h"
OBJECT_DECLARE_SIMPLE_TYPE(BANANA_ROM_MODULE, BANANA_ROM)

// Definição da estrutura que definirá o novo device

struct BANANA_ROM_MODULE{
...
static void Banana_ROM_read(void *opaque, hwaddr offset, unsigned size){
	
   uint64_t value = 0;
   BANANA_ROM_MODULE *s = opaque;
...
static void Banana_ROM_init(Object *obj)
{
    ...
    BANANA_ROM_MODULE *s = BANANA_ROM(obj);

    memory_region_init_io(&s->iomem, obj, &Banana_Rom_ops,
                          s, "BANANA_ROM_MODULE", 0x50);
...
static const TypeInfo Banana_ROM_info = {
    ...
    .instance_size = sizeof(BANANA_ROM_MODULE),
